...
const RbacCore = require('rbac-core-ng');
        ...
        if (config && config !== CONFIG_NONE) {
            try {
                ...
                if (result === RbacCore.DENY) {
                    return options.onError(request, h, new Boom.Boom('No permissions to access this resource', {
                        statusCode: options.responseCode.onDeny
                    }));
                }

                if (result === RbacCore.UNDETERMINED) {
                    return options.onError(request, h, new Boom.Boom('Could not evaluate access rights to resource', {
